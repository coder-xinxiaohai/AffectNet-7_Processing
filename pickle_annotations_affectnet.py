...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv_dir', type=str, default='./Manually_Annotated',
                        help='path to annotations of the dataset')
    ...
    for index, row in df_train.iterrows():
        ...
        sample = AffectNet_annotation(
            row['subDirectory_filePath'], row['face_x'], row['face_y'], row['face_width'], row['face_height'],
            row['expression'], row['valence'], row['arousal'], left_eye, right_eye)
    ...
    with open('./result/data_affectnet.pkl', "wb") as w:
        pickle.dump(data, w)
